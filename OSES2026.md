...
Capacity expansion models inform multi-billion dollar infrastructure decisions for offshore renewable energy systems. While parameter uncertainty is routinely addressed through sensitivity analyses and scenario methods, model uncertainty—the recognition that multiple valid framings of the same problem exist—receives comparatively little attention. This study proposes a methodology for empirically quantifying model-attributable uncertainty and distinguishing robust insights from modelling artifacts in offshore wind and storage planning.

We develop an open-source Python package that enables systematic comparison of OSeMOSYS and PyPSA, two widely-used capacity expansion frameworks with distinct design philosophies. OSeMOSYS represents energy systems as commodity-flows. Features such as net present value-based cost accounting, explicit salvage value treatment for long-lived assets, aggregation of wind variability in timeslice structures, and explicit reserve margin constraints, provide a transparent and computationally tractable framework with direct policy levers useful for national and regional policy planning contexts. PyPSA models energy systems as networks of components. Here, chronologically ordered snapshots preserve sequential storage dynamics and resource-demand correlations, storage units can be assigned explicit round-trip efficiency and self-discharge parameters, and unit commitment constraints and optional network physics provide the detailed operational representation that appeals to academic and industry applications.

Our translation framework distinguishes implementation disagreement—differences in implicit assumptions that can be harmonized through parameter mappings and pre- or post-processing adjustments—from structural disagreement, where models represent genuinely different but plausible problem framings. We apply this framework to a stylized North Sea offshore energy hub designed to isolate and quantify sources of model divergence relevant to investment decisions. The case study comprises an offshore wind farm with a representative capacity factor profile exhibiting characteristic North Sea variability, battery storage with configurable duration and efficiency parameters, a fixed-capacity grid connection to shore, and optional gas backup generation. Onshore demand follows a realistic profile with seasonal and diurnal variation correlated with wind resource availability.

We construct a progression of scenarios to systematically probe where model agreement holds and where it breaks down. A wind-only scenario tests how temporal aggregation affects curtailment estimates—PyPSA's hourly resolution captures periods when generation exceeds transmission capacity, while OSeMOSYS's timeslice averaging may smooth these peaks. Adding short-duration battery storage tests whether the models recommend similar storage capacity when intra-day arbitrage value depends on preserving hourly price spreads versus aggregating to representative day types. A long-duration storage variant (hydrogen or compressed air) examines whether OSeMOSYS's hierarchical seasonal structure adequately captures inter-seasonal shifting despite losing intra-seasonal dynamics.

We show that after harmonizing implementation differences, capacity recommendations for offshore wind will show closer agreement between frameworks than storage recommendations, where temporal representation fundamentally shapes the perceived value of flexibility. Specifically, OSeMOSYS tends to underestimate curtailment and short-duration storage requirements due to timeslice averaging of wind variability, while long-duration storage recommendations may converge more closely given OSeMOSYS's explicit seasonal structure. We also show that total system cost metrics will diverge more substantially than physical capacity recommendations, with the magnitude of economic divergence increasing for longer asset lifetimes relative to planning horizon—a finding that would have direct implications for how offshore wind investment cases are evaluated across different modelling frameworks.

Our translation framework and study design offers a template for treating capacity expansion models as members of an ensemble whose agreement signals robust insight and whose disagreement reveals where conclusions depend on modelling choices we might reasonably have made differently—enabling more honest uncertainty characterization when energy system models inform policy interventions and infrastructure investment.
